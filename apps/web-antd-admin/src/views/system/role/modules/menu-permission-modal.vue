...
watch(
  ...
  async (open) => {
    if (!open) return;
    checkedKeys.value = (props.role?.menuIds ?? []).map(Number);
    searchKeyword.value = '';
...
async function save() {
  ...
  try {
    const payload = {
      ...
      name: role.name,
      remark: role.remark,
      sort: (role as any).sort ?? 10,
      status: role.status,
    } as any;
    await updateRole(role.id, payload);
    message.success($t('system.role.menuSaveSuccess'));
    emits('success');
    close();
  } finally {
    saving.value = false;
...
<template>
  <Modal
    ...
    :open="open"
    :title="$t('system.role.configureMenus')"
    ...
    <div class="menu-permission-shell">
      <div class="menu-permission-summary">
        <div class="min-w-0">
          <div class="flex items-center gap-2">
            <span class="truncate text-base font-semibold">{{ role?.name }}</span>
            <Tag v-if="role?.isTenantAdmin" color="blue">
              {{ $t('system.role.tenantAdmin') }}
            </Tag>
          </div>
          <p class="text-muted-foreground mt-1 text-sm">
            {{ $t('system.role.menuPermissionHint') }}
          </p>
        </div>
        <div class="selected-count">
          <strong>{{ checkedKeys.length }}</strong>
          <span>{{ $t('system.role.selectedMenus') }}</span>
        </div>
      </div>
      ...
      <div class="menu-permission-toolbar">
        <Input.Search
          ...
          allow-clear
          :placeholder="$t('system.role.searchMenus')"
          class="max-w-sm"
        />
        <Space wrap>
      ...
      <Spin :spinning="loading">
        <div class="menu-tree-panel">
          ...
          <Empty v-else :description="$t('system.role.noMenusFound')" />
        </div>
      </Spin>

      <div class="menu-permission-impact">
        {{ $t('system.role.menuChangeImpact') }}
      </div>
    </div>
  </Modal>
</template>
